...
class io_object:
    ...
    For input assumes bin file has been converted to a mat file using matlab function: getBintoMat.m
    ...
    B) Generate header for writing processed data in a standard experiment format bin file. 
    ...
    def __init__(self, time_window, chunk_size, inp_file='[]', output_data_batch='', 
                     save_path='[]', bin_filename='data000000.bin', num_bits = 12, fs = 20000, overwrite = False):
        """
    ...
    def read_batch(self, batch_num, print_every):
        ...
        if self.chunk_size*(batch_num+1) <= self.num_samples:
            self.raw_data_batch = np.array(self.file['data'][:,self.chunk_size*batch_num:self.chunk_size*(batch_num+1)])
        else:
            self.raw_data_batch = np.array(self.file['data'][:,self.chunk_size*batch_num:self.num_samples])

        # If first batch, extract trigger properties and set the self.run_header flag to be true so that the header is added to bin file before addding processed data while writing. Note that 0th channel is trigger channel in raw data.
        if batch_num == 0:
            #print('Shape of a Raw Data Batch = ' + str(self.raw_data_batch.shape))
            #logging.info('Shape of a Raw Data Batch = ' + str(self.raw_data_batch.shape))
            print('Shape of a Raw Data Batch = ' + str(self.raw_data_batch.shape))
            #print(self.raw_data_batch[0,:])
...
#             import pdb; pdb.set_trace()            
        ...
        if batch_num%print_every == 0:
            ...
            #logging.info('Processing: Batch Number = ' + str(batch_num))
            print('Processing: Batch Number = ' + str(batch_num))
    ...
    def write_bin_file(self):
        ...
        if not os.path.exists(self.save_path):
            ...
            logging.info('Directory did not exist. Making a new one...')
